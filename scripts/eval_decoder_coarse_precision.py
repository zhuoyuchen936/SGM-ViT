#!/usr/bin/env python3
"""
scripts/eval_decoder_coarse_precision.py
========================================
Focused sweep for coarse-stage decoder precision in merged SGM-ViT.

This script continues the decoder-stage study by fixing the stage policy to
``coarse_only`` and sweeping:

- low-precision bit-width: INT4 / INT6
- high-precision region ratio: 0%, 25%, 50%, 75%

The goal is to quantify how far decoder-side coarse quantization can go before
the dense-only depth quality departs too much from the Merge FP32 baseline.
"""
...
def build_configs() -> list[dict[str, object]]:
    ...
    for low_bits in (6, 4):
        configs.append(
            {
                "key": f"decoder_coarse_int{low_bits}",
                "label": f"Decoder coarse INT{low_bits} proxy",
                "kind": "decoder",
                "hp_ratio": 0.0,
                "low_bits": low_bits,
            }
        )
        for hp_ratio in (0.25, 0.50, 0.75):
            hp_pct = int(round(hp_ratio * 100))
            configs.append(
                {
                    "key": f"decoder_coarse_int{low_bits}_caps{hp_pct}",
                    "label": f"Decoder coarse INT{low_bits} CAPS hp={hp_pct}%",
                    "kind": "decoder",
                    "hp_ratio": hp_ratio,
                    "low_bits": low_bits,
                }
...
def effective_precision_proxy(
    ...
    """
    Approximate a normalized precision budget for ``coarse_only``.

    Half the decoder stages are treated as active for this sweep, matching the
    coarse/fine split used in the stage-aware experiment.
    """
    active_fraction = 0.5
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.out_dir, "eval_decoder_coarse_precision.log")),
    ...
    logging.info("=" * 96)
    logging.info("SGM-ViT Decoder Coarse Precision Sweep")
    logging.info("=" * 96)
    ...
    logging.info(f"  High precision bits : {args.high_precision_bits}")
    logging.info(f"  Sweep LP bits       : INT6 / INT4")
    logging.info(f"  Sweep HP ratios     : 0.00 / 0.25 / 0.50 / 0.75")
    ...
    for idx, sample in enumerate(tqdm(samples, desc="Evaluating", unit="img")):
        ...
        for cfg in configs:
            if cfg["kind"] == "dense":
                ...
                proxy = 1.0
                low_bits = args.high_precision_bits
                hp_ratio = 1.0
            elif cfg["kind"] == "merge":
                ...
                depth_pred = run_decoder_caps_merged_da2(
                    ...
                    input_size=args.input_size,
                    merge_layer=args.merge_layer,
                    decoder_high_precision_ratio=hp_ratio,
    ...
    lines = [
        "=" * 96,
        "SGM-ViT Decoder Coarse Precision Sweep Results",
        "=" * 96,
    ...
    lines.extend(
        [
            "",
            "Readout:",
            "  - Compare INT6 vs INT4 at matched precision proxy to see whether modest low-bit coarse decoding is viable.",
            "  - Compare hp=0/25/50/75 to find the smallest high-precision region that stays near Merge FP32.",
            "  - If coarse INT6 is close to Merge FP32, decoder-side adaptive precision is likely a better next paper path than encoder-only adaptive precision.",
            "",
        ]
    )

    summary_path = os.path.join(args.out_dir, "decoder_coarse_precision_summary.txt")
    with open(summary_path, "w", encoding="utf-8") as handle:
        handle.write("\n".join(lines))
    logging.info("\n" + "\n".join(lines))
    logging.info(f"Summary: {summary_path}")

    csv_path = os.path.join(args.out_dir, "decoder_coarse_precision_per_image.csv")
    if csv_rows:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Focused coarse-stage decoder precision sweep for SGM-ViT.",
    )
    ...
    parser.add_argument(
        "--out-dir",
        default=default_results_dir("eval_decoder_coarse_precision"),
    )
